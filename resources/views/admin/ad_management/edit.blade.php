...
@section('content')
    <div class="py-12 px-5 2xl:mx-32 2xl:mt-48">
        <form action="{{ Route('admin.ad.management.update', $ad->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if (session('success'))
            <div class="p-2">
                            ...
                            <div class="grid grid-cols-2 gap-2 h-auto lg:h-96">
                                <div class="col-span-2">
                                    ...
                                    <input type="text"
                                        ...
                                        placeholder="Please enter ad name" name="name"
                                ...
                                <div class="col-span-2 md:col-span-1">
                                    ...
                                    <input type="text"
                                        class="border border-border-gray border-opacity-40 text-xs rounded-full w-full px-5 py-3 placeholder-dark-gray placeholder-opacity-40"
                                        placeholder="https://" name="link" value="{{ old('link', $ad->link) }}">
                                    @error('link')
                                        @include('errors.required_field', ['message' => $message])
                                    @enderror
                                </div>
                                <div class="col-span-2 md:col-span-1">
                                    ...
                                    <select id="id_select2_example" class="appearance-none w-full" name="country_id">
                                        @foreach ($country as $item)
                                            <option data-img_src="{{ URL::asset($item->image) }}" class="text-gray-400"
                                                value="{{ $item->id }}"
                                                {{ $ad->country_id == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('country_id')
                                ...
                                <div class="col-span-2 md:col-span-1">
                                    <label
                                        ...
                                        added</label>
                                    <div class="relative">
                                        <input type="text" id="ad_start_date_edit" autocomplete="off" name="date_added"
                                            ...
                                            placeholder="Start Date" value="{{ old('date_added', $ad->date_added) }}">
                                        <span class="absolute top-2 right-3.5 date-icon-ad-start-edit">
                                ...
                                <div class="col-span-2 md:col-span-1">
                                    <label
                                        ...
                                        ended</label>
                                    <div class="relative">
                                        <input type="text" id="ad_end_date_edit" autocomplete="off" name="date_ended"
                                            class="border border-border-gray border-opacity-40 uppercase text-xs rounded-full w-full px-5 py-3 placeholder-dark-gray placeholder-opacity-40"
                                            placeholder="End Date" value="{{ old('date_ended', $ad->date_ended) }}">
                                        <span class="absolute top-2 right-3.5 date-icon-ad-end-edit">
                                            <img src="{{ asset('assets/images/icons/black-calender-icon.svg') }}" alt="">
                                        </span>
                                    </div>
                                    @error('date_ended')
                                ...
                                <div class="col-span-2 md:col-span-1">
                                    ...
                                    <input type="number"
                                        class="border border-border-gray border-opacity-40 text-xs rounded-full w-full px-5 py-3 placeholder-dark-gray placeholder-opacity-40"
                                        placeholder="" name="cost" value="{{ old('cost', $ad->cost) }}">
                                    @error('cost')
                                        @include('errors.required_field', ['message' => $message])
                                    @enderror
                                </div>
                                <div class="col-span-2 md:col-span-1">
                                    ...
                                    <input type="number"
                                        ...
                                        placeholder="" name="extra_cost" value="{{ old('extra_cost', $ad->extra_cost) }}">
                                    @error('extra_cost')
                                        @include('errors.required_field', ['message' => $message])
                                    @enderror
                                </div>
                                <div class="col-span-2">
                                    ...
                                    <div class="relative items-center">
                                        <select
                                            ...
                                            name="visibility">
                                            <option {{ $ad->visibility == '1' ? 'selected' : '' }} value="1"
                                                class="text-gray-400">1</option>
                                            <option {{ $ad->visibility == '2' ? 'selected' : '' }} value="2"
                                                class="text-gray-400">2</option>
                                            <option {{ $ad->visibility == '3' ? 'selected' : '' }} value="3"
                                                class="text-gray-400">3</option>
                                            <option {{ $ad->visibility == '4' ? 'selected' : '' }} value="4"
                                                class="text-gray-400">4</option>
                                            <option {{ $ad->visibility == '5' ? 'selected' : '' }} value="5"
                                                class="text-gray-400">5</option>
                                        </select>
                    ...
                    <div class="md:px-6 lg:px-1 xl:px-6 mt-5 h-auto lg:h-96">
                        <div class="flex justify-center my-3">
                            <button
                                ...
                                <img class="w-5 h-5" src="{{ asset('assets/images/add-icon.svg') }}" alt=""
                                    id="thumbnil">
                                <input type="hidden" name="previous_img" value="{{ $ad->image }}">
                            </button>
                        </div>
                        @error('image')
                            @include('errors.required_field', ['message' => $message])
                        ...
                        <div class="md:px-6 lg:px-1 xl:px-6 py-5">
                            <img class="h-42 lg:h-[15rem] xl:h-[18rem] 2xl:h-72 rounded-lg object-cover"
                                src="{{ URL::asset($ad->image) }}" style="width: 100%;" id="thumbnilabc">
                        </div>
                    </div>
                    <div class="md:px-6 lg:px-1 xl:px-6 mt-2 lg:my-3 xl:mt-6 2xl:mt-11">
                        ...
                        <div class="relative items-center">
                            <select
                                ...
                                name="status">
                                <option {{ $ad->status == 'Published' ? 'selected' : '' }} value="Published"
                                    class="text-gray-400">Published</option>
                                <option {{ $ad->status == 'Unpublished' ? 'selected' : '' }} value="Unpublished"
                                    class="text-gray-400">Unpublished</option>
                            </select>
                            @error('status')
                                @include('errors.required_field', ['message' => $message])
                            @enderror
