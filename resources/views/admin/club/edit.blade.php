...
@section('content')
    <div class="py-12 px-5 md:mx-12 2xl:mx-32 2xl:mt-48">
        <form action="{{Route('admin.club.update', $club->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
                @if (session('success'))
                <div class="p-2">
                    ...
                    <div class="md:px-6 pt-8 md:pt-32 lg:py-24 xl:pt-36 2xl:pt-40">
                        <div class="mt-3">
                            ...
                            <div class="relative items-center">
                                <select
                                    ...
                                    <option value="{{ $item->id }}" {{ ($club->country_id) == $item->id ? 'selected' : '' }}>{{ $item->name}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mt-3">
                            ...
                            <div class="relative items-center">
                                ...
                                name="name" placeholder="Enter Club" autocomplete="off" value="{{$club->name}}">
                                @error('name')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mt-3">
                            ...
                            <div class="relative items-center">
                                <select
                                    ...
                                    <option  {{ ($club->status) == 'published' ? 'selected' : '' }} value="published">Published</option>
                                    <option {{ ($club->status) == 'unpublished' ? 'selected' : '' }} value="unpublished">Unpublished</option>
                                </select>
                                <span class="absolute inset-y-5 right-3.5">
                                    <img src="{{asset('assets/images/drop-arrow.svg')}}" alt="">
                                </span>
                    ...
                    <div class="md:px-6 pt-8 md:pt-32 lg:pt-36 xl:pt-44 2xl:pt-52">
                        <div class="flex justify-center my-3">
                                <div class="relative">
                                    ...
                                    <img class="w-12 h-12 -top-5 -right-2 md:-top-7 lg:-top-8 md:right-1" src="{{URL::asset($club->image)}}" alt="image" id="thumbnil">
                                    <input type="hidden" name="previous_img" value="{{$club->image}}">
                                </div>
                        </div>
                        @error('image')
