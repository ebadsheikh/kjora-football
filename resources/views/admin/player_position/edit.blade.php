...
@section('content')
    <div class="py-12 px-5 md:mx-12 2xl:mx-32 2xl:mt-48">
        <form action="{{Route('admin.player.position.update', $player_positions->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
                @if (session('success'))
                <div class="p-2">
                        ...
                        <div class="xl:px-6 py-5 md:py-40">
                            <div class="grid items-center grid-cols-1 md:grid-cols-2 md:gap-x-4 gap-y-2 2xl:gap-y-6">
                                <div class="col-span-2 md:col-span-1">
                                    <label
                                        class="uppercase block mb-1 2xl:mb-1.5 text-light-green text-xs font-bold px-5">Player Position</label>
                                    <input class="border border-border-gray border-opacity-40 text-xs rounded-full w-full px-5 py-3 placeholder-dark-gray placeholder-opacity-40"
                                        type="text" placeholder="IT ANALYST" name="name" value="{{old('name', $positions->name)}}">
                                        @error('name')
                                    ...
                                    <label
                                        ...
                                        <div class="relative items-center">
                                            <select
                                                ...
                                                <option  {{ ($positions->status) == 'Activate' ? 'selected' : '' }} value="Activate">Activate</option>
                                                <option {{ ($positions->status) == 'Deactivate' ? 'selected' : '' }} value="Deactivate">Deactivate</option>
                                            </select>
...
<script>
    ...
    new Chart("myChart", {
      ...
      options: {
        title: {
          display: true,
          text: "Total number of position "+{{$count_player_position}}
        }
      }
    });
    </script>
